...
\begin{document}
	...
	\noindent This study focuses on the development of a predictive model for analyzing students’ academic performance using Mathematical Modelling and Statistical Analysis techniques. The study examines the relationship between students’ GPA and selected factors such as study hours, attendance rate, number of courses taken, and genotype.
	
	A multiple linear regression model is developed using both real-life and simulated datasets collected across multiple semesters. Data preprocessing and regression analysis are implemented using Python computational tools. The model is evaluated and validated using statistical performance metrics and independent real-life data.
	
	The findings show that the selected variables significantly influence students’ academic performance and demonstrate the effectiveness of regression analysis in predicting academic outcomes. The study provides a data-driven framework that can support academic monitoring and educational decision-making.
	
	\vspace{1em}
	\noindent \textbf{Keywords:} Academic Performance, Grade Point Average (GPA), Regression Analysis, Predictive Modelling, Mathematical Modelling, Statistical Analysis, Multivariate Factors, Study Hours, Attendance Rate, Genotype, Educational Data Analysis, and Student Performance Prediction.
	...
	\begin{table}[htbp]
		...
		\begin{tabularx}{\textwidth}{@{}>{\raggedright\arraybackslash}p{2.4cm} >{\raggedright\arraybackslash}p{4.6cm} >{\raggedright\arraybackslash}X@{}}
			...
			Derived (trajectory) & Trajectory\_Slope, Trajectory\_Class & Per-student OLS slope and class (Improving / Stable / Declining) across CGPA100--400. \\
			\hline
		\end{tabularx}
	\end{table}
	...
	\begin{equation}
		\begin{split}
			\text{CGPA400}_i ={} & \beta_0 + \beta_1 \text{Previous\_GPA}_i
				+ \beta_2 \text{Attendance}_i + \beta_3 \text{StudyHours}_i \\
			& + \beta_4 \text{CourseLoad}_i + \beta_5 D_{\text{AS},i}
				+ \beta_6 D_{\text{SS},i} \\
			& + \beta_7 \bigl(D_{\text{SS},i} \times \text{Attendance}_i\bigr)
				+ \varepsilon_i,
		\end{split}
		\label{eq:full-model}
	\end{equation}
	...
	\section{Summary of Findings}
	Prior academic performance (\textit{Previous\_GPA}, the mean of Level~100--300 GPAs) was the strongest linear predictor of Level~400 GPA (CGPA400), with coefficient $\hat{\beta}_1 = 0.916$ ($p < 0.001$). This confirms that academic momentum is the primary driver of final-year outcomes, consistent with the broader educational research literature on cumulative performance.
	
	\textit{Attendance\_Rate} was the only behavioural variable to achieve statistical significance ($\hat{\beta}_2 = 0.005$, $p < 0.001$). While the effect size is small in absolute terms---a one percentage-point increase in attendance is associated with a 0.005-point increase in CGPA400---the finding is directionally consistent with the literature reviewed in Chapter~2 and supports the inclusion of attendance as a meaningful covariate. Study hours, course load, and genotype dummies were not significant at $\alpha = 0.05$, consistent with the weak synthesis signal and the small SS subsample ($n_{\text{SS}} = 37$ in the modelling sample).
	
	The decision to reframe the dependent variable from overall CGPA to CGPA400 reduced the adjusted $R^2$ from approximately 0.95 to 0.646, eliminating the arithmetic overlap between \textit{Previous\_GPA} and the outcome. Five-fold cross-validation yielded out-of-fold MAE $= 0.378$ and RMSE $= 0.478$ on a 0--5 scale, representing a 42\% reduction in mean absolute error relative to the naive baseline (MAE $= 0.652$). This is an honest and reproducible predictive result.
	...
	The health-pathway genotype inclusion---in which SS students were assigned a mean attendance penalty of 7.7 percentage points relative to AA students under the synthesis assumptions---produced the expected directional pattern in the data. The SS $\times$ Attendance interaction term ($\hat{\beta}_7 = -0.004$, $p = 0.671$) was not significant, which is attributed to the small SS subsample rather than absence of the pathway. This result is illustrative and consistent with the stated design intent; it does not constitute clinical evidence of a genotype effect on academic performance.
