...
\begin{document}
	...
	\textbf{Dependent variable:} CGPA, the overall cumulative grade point average at graduation.
	
	\textbf{Previous\_GPA:} $\text{Previous\_GPA} = (\text{CGPA100} + \text{CGPA200} + \text{CGPA300})/3$, rounded to four decimal places. This is the mean performance over the first three academic levels and is temporally prior to Level~400 outcomes embedded in final CGPA.
	...
	Genotype is assigned by an independent multinomial draw with probabilities $P(\text{AA}) = 0.75$, $P(\text{AS}) = 0.24$, $P(\text{SS}) = 0.01$, reflecting illustrative Nigerian population priors for haemoglobin genotypes. The draw is \textit{not} conditioned on CGPA and is \textit{not} taken from student health records. In regression, AA is the reference category; AS and SS enter as dummy indicators \texttt{Genotype\_AS} and \texttt{Genotype\_SS}.
	
	\section{Regression Model Specification}
	The empirical multiple linear regression model is:
	\begin{equation}
		\text{CGPA}_i = \beta_0 + \beta_1 \text{Previous\_GPA}_i + \beta_2 \text{Attendance}_i + \beta_3 \text{StudyHours}_i + \beta_4 \text{CourseLoad}_i + \beta_5 D_{\text{AS},i} + \beta_6 D_{\text{SS},i} + \varepsilon_i,
		\label{eq:full-model}
	\end{equation}
	where $D_{\text{AS}}$ and $D_{\text{SS}}$ are one if genotype is AS or SS respectively, and zero otherwise (reference: AA). Equation~(\ref{eq:full-model}) extends the three-predictor specification in Chapter~1 with lagged academic history and course load while retaining study hours, attendance, and genotype as stated in the research objectives.
	
	OLS assumptions (linearity, independent errors, homoskedasticity, and approximate normality of residuals) were assessed graphically via residuals-versus-fitted, Q--Q, and scale-location plots (Chapter~4). Variance inflation factors (VIF) for all predictors were below 10, indicating no severe multicollinearity in the fitted design matrix.
	...
	Seven limitations govern interpretation: (L1)~residual endogeneity in blended behavioural synthesis; (L2)~arithmetic overlap between \textit{Previous\_GPA} and CGPA; (L3)~synthesized behavioural variables are not observed; (L4)~overload rate and course ceiling assumptions; (L5)~ID collisions in source data; (L6)~parameters calibrated for MTU/Nigeria; (L7)~synthesized genotype, not clinical records. These are discussed again with results in Chapter~4.
	...
	Pearson correlations with CGPA were: \textit{Previous\_GPA} $r = 0.976$; \textit{Attendance\_Rate} $r = 0.285$; \textit{Study\_Hours\_Per\_Week} $r = 0.238$; \textit{Course\_Load} $r = -0.034$. The very strong association for \textit{Previous\_GPA} reflects definitional overlap with CGPA (Limitation L2), not solely empirical discovery. Genotype, synthesized independently of CGPA, showed negligible linear association with CGPA ($|r| < 0.15$ on an ordinal genotype proxy), as intended by the generative design.
	
	\section{Regression Results}
	Table~\ref{tab:regression} reports OLS coefficients for Equation~(\ref{eq:full-model}). Adjusted $R^2 = 0.953$. \textit{Previous\_GPA} has coefficient $\hat{\beta}_1 \approx 0.964$ ($p < 0.001$): a one-point increase in the three-year mean GPA is associated with approximately a 0.96-point higher CGPA, holding other predictors fixed. Attendance and study hours are positive and statistically significant but small in magnitude ($\hat{\beta}_2 \approx 0.002$, $\hat{\beta}_3 \approx 0.006$ per unit). Course load is near zero ($\hat{\beta}_4 \approx -0.003$, $p \approx 0.016$). Genotype dummies are not significant (AS: $\hat{\beta}_5 \approx -0.0002$, $p \approx 0.98$; SS: $\hat{\beta}_6 \approx 0.014$, $p \approx 0.60$), consistent with independent synthesis and weak overlap with CGPA.
	
	\begin{table}[htbp]
		\centering
		\caption{OLS regression of CGPA on predictors ($n = 2{,}974$; adjusted $R^2 = 0.953$)}
		\label{tab:regression}
		\begin{tabular}{lrrr}
			\hline
			Predictor & Coefficient & $p$-value & VIF \\
			\hline
			Previous\_GPA & 0.964 & $< 0.001$ & 1.13 \\
			Attendance\_Rate & 0.002 & $< 0.001$ & 1.08 \\
			Study\_Hours\_Per\_Week & 0.006 & $< 0.001$ & 1.06 \\
			Course\_Load & $-0.003$ & 0.016 & 1.00 \\
			Genotype (AS vs AA) & $-0.0002$ & 0.977 & $< 1.2$ \\
			Genotype (SS vs AA) & 0.014 & 0.597 & $< 1.2$ \\
			\hline
		\end{tabular}
	\end{table}
	
	\section{Model Diagnostics}
	Residual plots (residuals versus fitted values, normal Q--Q, histogram, and scale-location) are produced in \texttt{model.ipynb} and the Streamlit OLS tab. Residuals cluster tightly around zero because \textit{Previous\_GPA} explains most of the variance; well-behaved diagnostics therefore reflect the dominant lagged-GPA term (L2) as much as model adequacy for behavioural predictors. No predictor exhibited VIF $\geq 10$.
	
	\section{In-Sample Fit}
	The actual-versus-predicted CGPA scatter lies close to the 45$^\circ$ identity line (in-sample fitted values). This tight fit is expected given $r = 0.976$ between \textit{Previous\_GPA} and CGPA and should not be cited as proof that attendance or study hours drive final GPA in the data-generating process (L1, L3).
	
	\section{Out-of-Sample Predictive Performance}
	Five-fold cross-validation (seed 42) yielded stacked out-of-fold MAE $= 0.113$, RMSE $= 0.150$, and $R^2 = 0.953$. The mean-only baseline achieved MAE $= 0.567$ and RMSE $= 0.690$, showing that the regression specification substantially reduces prediction error relative to guessing the sample mean, though most of that gain is attributable to \textit{Previous\_GPA}. These metrics support the secondary predictive objective under the stated synthesis assumptions; they do not validate causal claims about attendance, study hours, or genotype.
	
	\section{Time-Series Analysis of Per-Level GPA}
	Using observed CGPA100--CGPA400, mean GPA trajectories were computed by programme and by year of graduation. Each student contributes one GPA per level; group lines are cross-sectional means at each level, not a single cohort tracked over calendar time. Programme heatmaps and boxplots (notebook Section~4 and Streamlit Trajectory tab) show heterogeneity across programmes and levels---for example, relative ordering of programmes at Level~400. These analyses use only observed per-level GPAs and do not depend on synthesized predictors.
	
	\section{Predictive Use Case}
	The scoring utility accepts new rows with the predictors in Table~\ref{tab:data-sources} and returns \texttt{predicted\_CGPA}. Example: a student with CGPA100--300 means implying \textit{Previous\_GPA} $\approx 3.37$, attendance 85\%, 10 study hours per week, 12 courses, and genotype AA receives a predicted CGPA of approximately 3.55 under the refitted model. The Streamlit \textit{Predict \& CV} tab exposes the same logic interactively.
	
	\section{Summary of Findings}
	\begin{itemize}
		\item Prior three-year academic performance (\textit{Previous\_GPA}) is the dominant linear predictor of final CGPA.
		\item Synthesized attendance and study hours show weak positive associations; practical effect sizes are small once \textit{Previous\_GPA} is controlled.
		\item Course load and synthesized genotype add negligible explanatory power in this specification.
		\item Out-of-sample errors are low (MAE $\approx 0.11$ on a 5-point scale) but largely reflect lagged GPA information.
		\item Per-level trajectory analysis reveals programme and cohort patterns in observed GPA series.
	\end{itemize}
	
	\section{Limitations of the Results}
	\label{sec:limitations-results}
	\textbf{L1 (Endogeneity):} Attendance and study hours retain 40\% CGPA-linked structure in synthesis; significant coefficients do not prove causal effects. \textbf{L2 (Overlap):} \textit{Previous\_GPA} shares components with CGPA; adjusted $R^2$ is not a pure behavioural explanation. \textbf{L3 (Synthesis):} Behavioural and load variables were not measured. \textbf{L4 (Overload):} 5\% overload rate is assumed. \textbf{L5 (IDs):} Duplicate ID collisions in source data. \textbf{L6 (Context):} MTU-specific calibration. \textbf{L7 (Genotype):} Genotype is simulated, not clinically recorded. Results are illustrative for methodology demonstration and institutional discussion, not registrar-certified forecasting without observed predictors.
